...
def status(output_json: bool):
    ...
    console.print(f"[bold]Cache:[/bold] [{cache_style}]{cache['status'].upper()}[/{cache_style}] - {cache['message']}")
    ...
    console.print(f"[bold]Overall:[/bold] [{overall_style}]{overall.upper()}[/{overall_style}] ({healthy}/{total} scrapers healthy)")
